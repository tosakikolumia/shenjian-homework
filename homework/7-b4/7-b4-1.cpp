...
public:
	...
	double area()
	{
		if (((p2.getx() - p1.getx()) * (p3.gety() - p1.gety()) - (p2.gety() - p1.gety()) * (p3.getx() - p1.getx())) == 0) {//det = (x2 - x1) * (y3 - y1) - (y2 - y1) * (x3 - x1)
			return -1;
		}
		double chaji = fabs((double(p2.getx()) - p1.getx()) * (double(p3.gety()) - p1.gety()) - (double(p2.gety()) - p1.gety()) * (double(p3.getx()) - p1.getx()));//叉积
		double s = chaji / 2;
		return s;
	}
	/* 构造函数 */
