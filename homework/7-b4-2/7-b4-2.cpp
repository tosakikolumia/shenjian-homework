...
public:
	...
	double area()
	{
		if (((p2.x - p1.x) * (p3.y - p1.y) - (p2.y - p1.y) * (p3.x - p1.x)) == 0) {//det = (x2 - x1) * (y3 - y1) - (y2 - y1) * (x3 - x1)
			return -1;
		}
		double chaji = fabs((double(p2.x) - p1.x) * (double(p3.y) - p1.y) - (double(p2.y) - p1.y) * (double(p3.x) - p1.x));//叉积
		double s = chaji / 2;
		return s;
	}
	/* 构造函数 */
