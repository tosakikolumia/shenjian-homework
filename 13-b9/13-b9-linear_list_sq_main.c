...
#ifdef ELEMTYPE_IS_DOUBLE
        printf("在插入元素=%5.1f,前插入前驱元素=%5.1f成功\n",  e1, e2);
#elif defined (ELEMTYPE_IS_CHAR_ARRAY) || defined (ELEMTYPE_IS_CHAR_P)
        printf("在插入元素=%s,前插入前驱元素=%s成功\n",  e1, e2);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)  
        printf("在学号=\"%d\",前插入前驱元素=\"%d.%s.%c.%f.%s\"成功\n", e1.num, e2.num, e2.name, e2.sex, e2.score, e2.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
        printf("在学号=\"%d\",前插入前驱元素=\"%d.%s.%c.%f.%s\"成功\n",  e1->num, e2->num, e2->name, e2->sex, e2->score, e2->addr);
#else	//缺省当做int处理
        printf("在插入元素=%d,前插入前驱元素=%d成功\n", e1, e2);
#endif
        printf("新表为：\n");
        ListTraverse(L, MyVisit);
    }
...
#ifdef ELEMTYPE_IS_DOUBLE
        printf("在插入元素=%5.1f,前插入前驱元素=%5.1f失败\n",  e1, e2);
#elif defined (ELEMTYPE_IS_CHAR_ARRAY) || defined (ELEMTYPE_IS_CHAR_P)
        printf("在插入元素=%s,前插入前驱元素=%s失败\n",  e1, e2);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
        printf("在学号=\"%d\",前插入前驱元素=\"%d.%s.%c.%f.%s\"失败\n", e1.num, e2.num, e2.name, e2.sex, e2.score, e2.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
        printf("在学号=\"%d\",前插入前驱元素=\"%d.%s.%c.%f.%s\"失败\n", e1->num, e2->num, e2->name, e2->sex, e2->score, e2->addr);
#else	//缺省当做int处理
        printf("在插入元素=%d,前插入前驱元素=%d失败\n",  e1, e2);
#endif
